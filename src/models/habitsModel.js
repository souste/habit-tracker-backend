...
async function deleteHabitModel(id, userId) {
  const result = await pool.query(`DELETE FROM habits WHERE id = $1 AND user_id = $2 RETURNING id`, [id, userId]);

  return result.rows[0];
}

module.exports = {
  getHabitsByUserModel,
  createHabitModel,
  updateHabitModel,
  deleteHabitModel,
};
